...
public class Program : IPlugin
{
    ...
    public static Client cli;
     ...
     #region Define IPlugin
        ...
        { return "Port of 'JeffTheAi' to KRelay"; }

        public string[] GetCommands()
        { return new string[] { @"/jeff on:off
say something with '#' at the start to talk to jeff" }; }

        #endregion

        public void Initialize(Proxy proxy)
        {
            proxy.HookCommand("#", Input);
            proxy.HookCommand("jeff", OnMyPluginCommand);
            proxy.HookPacket(PacketType.PLAYERTEXT, TEXT);
        }

        public static void OnMyPluginCommand(Client client, string command, string[] args)
        {
            if (args[0].Contains("on") == true)
            {
                enabled = true;
        ...
        public static void Input(Client client, string command, string[] args)
        {
            if (listen == true)
            {
                wotsay = string.Join(" ", args);
                listen = false;
            }
        }
    ...
    public static void TEXT(Client client, Packet packet)
    {
    ...
    {
        ConsoleWriteLine("Hello! I'm Jeff");
        /*
    ...
    public static void defs()
    {
      ...
      //  if (wotsay.ToLower().Contains("therefore") == true && wotsay.ToLower().Contains("either") == true && wotsay.ToLower().Contains("way") == true) { Console.WriteLine("Either way, therefore, I think that is very interesting"); thereforemode = !thereforemode; }
        ...
        if (wotsay == "learn")
        {
            ConsoleWriteLine("Welcome to the Learning Wizzard - here you can teach me new words."); 
            fin("If you say text to me that contains a keyword - you can tell me what to reply to it with");
            ConsoleWriteLine("Oh and also - you can define multiple keywords - the answer shall be done for any of the keywords");
            ConsoleWriteLine("and you can do multiple answers too - a random answer will be given...");
            ConsoleWriteLine("seperate them by a '`' sign");
            Console.ForegroundColor = ConsoleColor.Gray;
            Console.ReadLine();
            Console.ForegroundColor = ConsoleColor.Yellow;
            Console.Clear();
            ...
            Console.ForegroundColor = ConsoleColor.Yellow;
            writeq(tmp1.ToLower());
            ConsoleWriteLine("Enter Reply(s):");
            Console.ForegroundColor = ConsoleColor.Gray;
            listen = true;
            string tmp2 = wotsay;
            Console.ForegroundColor = ConsoleColor.Yellow;
            writesay(tmp2);
            Console.Clear();
            ConsoleWriteLine("Got it! From now on, if you say something to me that contains '" + tmp1.ToLower() + "' in it, I shall reply with '" + tmp2 + "'");
            scan();
            defs();
        }
        #endregion
    ...
    {
        ConsoleWriteLine("rectangles are not yet supported sorry");
        ChangeSubject();
        /*
    ...
    public static void recite()
    {
        ...
        {
            Console.ForegroundColor = ConsoleColor.Yellow;
            ConsoleWriteLine("What a strange question - I think that ");
        }
        if (wotsay.ToLower().Contains("joke") == true)
        {
            Console.ForegroundColor = ConsoleColor.Yellow;
            ConsoleWriteLine("You want a joke? This request ... I think that ");
        }
        //CHANGE SUBJECT
        ChangeSubject();
    }
    ...
    {
        Console.ForegroundColor = ConsoleColor.Yellow;
        Console.WriteLine("Intresting... however this leads me to wonder... ");
        string wh7 = "Wh7NaN";
        string art = "artNaN";
        string nom = "nomNaN";
        string stat = "statNaN";
    ...
    {
        Console.ForegroundColor = ConsoleColor.Green;
        ConsoleWriteLine("Scanning .jeff files:");
        pause(300);
        int counter = 0;
        string line;
        ...
        {
            ConsoleWriteLine("Invalid .jeff files! Cannot load");
            Environment.Exit(1);
        }
        //else continue forth to store vars:
        ConsoleWriteLine(".jeff files found!");
        ConsoleWriteLine("no. args = " + counteru);
        pause(300);
    ...
    public static void learn(string que)
    {
       ...
       // say = new string[say.Length+1];
        ...
        writeq(que.ToLower());
        ConsoleWriteLine("LEARNT: " + que + " = " + wotsay);
        pause(500);
    ...
    public static void reply(string wha, string rawans)
    {
        ...
        {Console.Clear();ConsoleWriteLine(answer[rand]);no=0;defs();}{Console.Clear();/*exep(1);*/
    ...
    {
        cli.SendToClient(PluginUtils.CreateOryxNotification("Jeff", txt));
    }
}